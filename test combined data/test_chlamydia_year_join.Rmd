...
code_state <- read_sf("code_state.geojson")
chlamydia_test <- read.csv("chlamydia_test.csv")
```
...
```{r}
chlamydia23yd <- chlamydia23y[-1,]

```
```{r}
write.csv(chlamydia23yd, "chlamydia_test.csv", row.names = FALSE)


join <-
  code_state %>%
  left_join(chlamydia_test,
            by = c("name"))
```
```{r}
write.csv(join, "state_test.csv", row.names = FALSE)
```
